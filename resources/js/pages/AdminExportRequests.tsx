...
const AdminExportRequests = () => {
  ...
  const fetchRequests = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await api.get("/admin/export-requests", { params: { status: tab } });
  ...
  const visibleTabs = isManager ? (["pending", "approved", "rejected"] as const) : TABS;
  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-slide-in">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-xs sm:text-sm mt-1">
            {isManager
              ...
              : "Review manager-requested data exports. Approvals expire after 24 hours."}
          </p>
        </div>
        {!isManager && (
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 gap-1.5 sm:flex sm:gap-2">
        {visibleTabs.map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <div className="relative flex-1">
          ...
          <Input
            placeholder={isManager ? "Search your requests…" : "Search by manager name, username, or email"}
            value={search}
